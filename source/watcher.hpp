            ...
            */
            ...
            struct event {
                    ...
                    */
                    ...
                    event(const char* where, const enum what happen,
                          const enum kind kind)
                        : where{where},
            ...
            namespace adapter {
                ...
                */
                ...
                static bool can_watch() {
                    if (watcher_alive) return false;
                    else watcher_alive = true;
                    return true;
                }
                ...
                */
                ...
                static bool die(const event::callback& callback) {
                    if (watcher_alive) {
                        watcher_alive = false;
                        callback(event::event{"s/self/die",
                                              event::what::destroy,
                                              event::kind::watcher});
                        return true;
                    } else {
                        callback(event::event{"e/self/die",
                                              event::what::destroy,
                                              event::kind::watcher});
                        return false;
                    }
        ...
        namespace detail {
            namespace adapter {
                namespace {
                    ...
                    using flag_pair =
                        std::pair<FSEventStreamEventFlags, event::what>;
    ...
    flag_pair(kFSEventStreamEventFlagItemCloned,         event::what::other),
                    ...
                    auto mk_event_stream(const char* path,
                        ...
                        /*  the contortions here are to please darwin.
                            importantly, `path_as_refref` and its underlying
                           types *are* const qualified. using void** is not ok.
                           but it's also ok. */
                        const void* _path_ref = CFStringCreateWithCString(
                            nullptr, path, kCFStringEncodingUTF8);
                        const void** _path_refref{&_path_ref};

                        /* We pass along the path we were asked to watch, */
                        const auto translated_path = CFArrayCreate(
                            nullptr,               /* not sure */
                            _path_refref,          /* path string(s) */
                            1,                     /* number of paths */
                            &kCFTypeArrayCallBacks /* callback */
                        );
                        /* the time point from which we want to monitor events
                         * (which is now), */
                        ...
                        const auto event_stream_flags =
                            kFSEventStreamCreateFlagFileEvents |
                            kFSEventStreamCreateFlagUseExtendedData |
                            kFSEventStreamCreateFlagUseCFTypes |
                            kFSEventStreamCreateFlagNoDefer;
                        /* to the OS, requesting a file event stream which uses
                         * our callback. */
                        return FSEventStreamCreate(
                            nullptr,         /* allocator */
                            callback,        /* callback; what to do */
                            nullptr,         /* context (see note [event stream
                                                context]) */
                ...
                static bool watch(const char* path,
                    ...
                    const auto callback_adapter =
                        [](ConstFSEventStreamRef, /* stream_ref
                           ...
                           const FSEventStreamEventId*) {
                            auto decode_flags =
                                [](const FSEventStreamEventFlags& flag_recv) {
                                    std::vector<event::what> translation;
                                    /* @todo this is a slow, dumb search. fix
                                     * it. */
                                    for (const flag_pair& it :
                                         flag_pair_container)
                            ...
                            for (decltype(os_event_count) i = 0;
                                ...
                                for (auto const& flag_it :
                                     decode_flags(os_event_flags[i]))
                                    if (translated_path != nullptr)
                                        callback_hook(
                                            water::watcher::event::event{
                                                translated_path, flag_it,
                                                discern_kind(translated_path)});
                            }
                        };

                    const auto alive_os_ev_queue =
                        [](const FSEventStreamRef& event_stream,
                    ...
                    const auto event_stream = mk_event_stream<delay_ms>(
                        path, callback_adapter);

                    /* request a high priority queue */
                    const auto event_queue = dispatch_queue_create(
                        "water.watcher.event_queue",
                        dispatch_queue_attr_make_with_qos_class(
                            DISPATCH_QUEUE_SERIAL, QOS_CLASS_USER_INITIATED,
                            -10));

                    if (alive_os_ev_queue(event_stream, event_queue))
                        while (is_living())
                            /* this does nothing to affect processing, but this
                               thread doesn't need to run an infinite loop
                               aggressively. It can wait, with some latency,
                ...
                To set up a context with some parameters, something like this,
                from the `fswatch` project repo, could be used:
                ...
                To grab the inode and time information about an event, something
                like this, also from `fswatch`, could be used:
        ...
        namespace detail {
            namespace adapter {
                namespace {
                    ...
                    /* @brief
                       water/watcher/detail/adapter/linux/<anonymous>/types
                       Types:
                         - dir_opt_type
                         - path_container_type */
                    using dir_opt_type = std::filesystem::directory_options;
                    using path_container_type =
                        std::unordered_map<int, std::string>;

                    /* @brief
                       water/watcher/detail/adapter/linux/<anonymous>/constants
                       Constants:
                         - event_max_count
                         - in_init_opt
                         - in_watch_opt
                    ...
                    /* @todo
                       Handle move events properly.
                         - Use IN_MOVED_TO
                         - Use event::<something> */
                    inline constexpr auto in_watch_opt = IN_CREATE | IN_MODIFY |
                                                         IN_DELETE |
                                                         IN_MOVED_FROM |
                                                         IN_Q_OVERFLOW;
                    ...
                    /* @brief
                       water/watcher/detail/adapter/linux/<anonymous>/functions
                       Functions:
                         - do_scan */

                    /* @brief
                       water/watcher/detail/adapter/linux/<anonymous>/functions/do_scan
                    ...
                    inline bool do_scan(int fd,
                        ...
                        enum class event_recv_status {
                            eventful,
                            eventless,
                            error
                        };

                        const auto lift_event_recv = [](int fd, char* buf) {
                            /* Read some events. */
                            ...
                            if (len < 0 && errno != EAGAIN)
                                return std::make_pair(event_recv_status::error,
                                                      len);
                            else if (len <= 0)
                                return std::make_pair(
                                    event_recv_status::eventless, len);
                            else
                                return std::make_pair(
                                    event_recv_status::eventful, len);
                        };

                        /* Loop while events can be read from the inotify file
                         * descriptor. */
                        while (true) {
                            /* Read events */
                            ...
                            switch (status) {
                                case event_recv_status::eventful:
                                    ...
                                    for (char* ptr = buf; ptr < buf + len;
                                         ...
                                         */
                                        const auto path_kind =
                                            event_recv->mask & IN_ISDIR ?
                                                event::kind::dir :
                                                event::kind::file;
                                        int path_wd = event_recv->wd;
                                        auto event_base_path =
                                            path_container.find(path_wd)
                                                ->second;
                                        auto event_path = std::string(
                                            event_base_path + "/" +
                                            event_recv->name);
                                        ...
                                        if (event_recv->mask & IN_Q_OVERFLOW)
                                            ...
                                            callback(event::event{
                                                event_path.c_str(),
                                                event::what::other, path_kind});
                                    }
                                    break;
                                case event_recv_status::error:
                                    callback(event::event{
                                        "e/sys/read", event::what::other,
                                        event::kind::watcher});
                                    return false;
                                    break;
                ...
                static bool watch(const char* base_path,
                                  const event::callback& callback) {
                    /*
                       Functions
                     */
                    ...
                    const auto&& do_path_container_create =
                        [](const char* base_path_c_str, const int watch_fd,
                           const event::callback& callback)
                        ...
                        auto do_mark = [&](auto& dir) {
                            int wd = inotify_add_watch(watch_fd, dir.c_str(),
                                                       in_watch_opt);
                            return wd < 0 ?
                                       [&]() {
                                           callback(event::event{
                                               "e/sys/inotify_add_watch",
                                               event::what::other,
                                               event::kind::watcher});
                                           return false;
                                       }() :
                        ...
                        if (!do_mark(base_path)) return std::nullopt;
                        else if (std::filesystem::is_directory(base_path,
                                                               dir_ec))
                            /* @todo @note
                               Should we bail from within this loop if `do_mark`
                               fails? */
                            for (auto const& dir :
                                 rdir_iterator(base_path, dir_opt, dir_ec))
                                if (!dir_ec)
                    ...
                    /* Return and initializes epoll events and file descriptors.
                       ...
                       Or return nothing. */
                    const auto&& do_event_resource_create =
                        [](const int watch_fd, const event::callback& callback)
                        -> std::optional<
                            std::tuple<epoll_event, epoll_event*, int>> {
                        ...
                        if (epoll_ctl(event_fd, EPOLL_CTL_ADD, watch_fd,
                                      &event_conf) < 0) {
                            callback(event::event{"e/sys/epoll_ctl",
                                                  event::what::other,
                                                  event::kind::watcher});
                            return std::nullopt;
                        } else
                            return std::move(std::make_tuple(
                                event_conf, event_list, event_fd));
                    };

                    /* Return an optional file descriptor
                       which may access the inotify api. */
                    const auto do_watch_fd_create =
                        [](const event::callback& callback)
                        -> std::optional<int> {
                        int watch_fd
#if defined(WATER_WATCHER_PLATFORM_LINUX_ANY)
                        ...
                        if (watch_fd < 0) {
                            callback(event::event{"e/sys/inotify_init1",
                                                  event::what::other,
                                                  event::kind::watcher});
                            return std::nullopt;
                        } else return watch_fd;
                    };

                    /* Close the file descriptor `fd_watch`,
                       Invoke `callback` on errors. */
                    const auto do_watch_fd_release =
                        [](int watch_fd, const event::callback& callback) {
                            if (close(watch_fd) < 0) {
                                callback(event::event{"e/sys/close",
                                                      event::what::other,
                                                      event::kind::watcher});
                                return false;
                            } else return true;
                        ...
                        [](int event_fd, auto& event_list, int watch_fd,
                           ...
                           const event::callback& callback) {
                            ...
                            auto const do_event_error = [&]() {
                                callback(event::event{"e/sys/epoll_wait",
                                                      event::what::other,
                                                      event::kind::watcher});
                            ...
                            return event_count < 0 ? do_event_error() :
                                                     do_event_dispatch();
                        };
                    ...
                    if (watch_fd_optional.has_value()) {
                        ...
                        auto&& path_container_optional =
                            do_path_container_create(base_path, watch_fd,
                                                     callback);

                        if (path_container_optional.has_value()) {
                            /* Find all directories above the base path given.
                               Make a map of watch descriptors -> paths. */
                            path_container_type&& path_container = std::move(
                                path_container_optional.value());

                            auto&& event_tuple = do_event_resource_create(
                                watch_fd, callback);

                            if (event_tuple.has_value()) {
                                /* auto&& event_conf =
                                 * std::get<0>(event_tuple.value()); */
                                auto&& event_list = std::get<1>(
                                    event_tuple.value());
                                auto&& event_fd = std::get<2>(
                                    event_tuple.value());
                                ...
                                while (is_living() &&
                                       do_event_wait_recv(
                                           event_fd, event_list, watch_fd,
                                           path_container, callback))
                                    continue;
                                return do_watch_fd_release(watch_fd, callback);
                            } else
                                return do_watch_fd_release(watch_fd, callback);
                        } else return do_watch_fd_release(watch_fd, callback);
                    } else return false;
                }
...
using bucket_type = std::unordered_map<std::string, std::filesystem::file_time_type>;
                    ...
                    static bool scan(const char* path, const auto& send_event,
                                     bucket_type& bucket) {
                        ...
                        const auto scan_file =
                            [&](const char* file,
                                const auto& send_event) -> bool {
                            ...
                            if (exists(file) && is_regular_file(file)) {
                                ...
                                auto const timestamp = last_write_time(file,
                                                                       ec);
                                if (ec) {
                                    /* the file changed while we were looking at
                                     * it. so, we call the closure, indicating
                                     * destruction, and remove it from the
                                     * bucket. */
                                    send_event(
                                        event::event{file, event::what::destroy,
                                                     event::kind::file});
                        ...
                        const auto scan_directory =
                            ...
                            if (is_directory(dir)) {
                                ...
                                for (auto const& file :
                                     recursive_directory_iterator(dir, dir_opt,
                                                                  dir_it_ec))
                                    /* while handling errors */
                                    if (dir_it_ec) return false;
                                    else /* msvc complains without this cast */
                                        scan_file(
                    ...
                    static bool tend_bucket(const char* path,
                        ...
                        const auto populate = [&](const char* path) -> bool {
                            using std::filesystem::exists,
                                ...
                                std::filesystem::recursive_directory_iterator,
                                std::filesystem::last_write_time;
                            /* this happens when a path was changed while we
                             were reading it. there is nothing to do here; we
                             prune later. */
                            auto dir_it_ec = std::error_code{};
                                ...
                                if (is_directory(path)) {
                                    for (auto const& file :
                                         recursive_directory_iterator(
                                             path, dir_opt, dir_it_ec)) {
                                        if (!dir_it_ec) {
                                            auto const lwt = last_write_time(
                                                file, lwt_ec);
                                            if (!lwt_ec)
                                                bucket[file.path().string()] =
                                                    lwt;
                                            else
                                                /* @todo use this practice
                                                   elsewhere or make a fn for it
                         ...
                         */
                        const auto prune = [&](const char* path,
                                               const auto& send_event) -> bool {
                            using std::filesystem::exists,
                                std::filesystem::is_regular_file,
                                std::filesystem::is_directory,
                            ...
                            while (bucket_it != bucket.end()) {
                                /* check if the stuff in our bucket exists
                                 * anymore. */
                                exists(bucket_it->first)
                                    /* if so, move on. */
                                    ?
                                    std::advance(bucket_it, 1)
                                    /* if not, call the closure, indicating
                                       destruction, and remove it from our
                                       bucket. */
                                    :
                                    [&]() {
                                        send_event(event::event{
  ...
  */
                ...
                // In the future, we could allow overloads and defaults like
                // this.

                // static bool watch(const char* path, event::callback const&
                // callback) {
                //   return watch(path, callback,
                //                []() -> bool { return watcher_alive ? true :
                //                false; });
                // }
...
namespace water {
    namespace watcher {
        ...
        */
        ...
        static bool watch(const char* path, const event::callback& callback) {
